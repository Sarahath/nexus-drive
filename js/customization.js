...
/* ─── ROTATE-DEVICE PROMPT ────────────────────────────────────────
   Covers the gap the Orientation Lock/Fullscreen APIs leave (mainly
   iOS Safari, which has neither) by guiding the player to rotate
   manually. Purely a CSS overlay driven by the browser's own
   orientation media query — this function only wires the optional
   "Continue in Portrait" dismissal and re-arms it on the next
   portrait/rotate event so it isn't permanently silenced. */
function wireRotatePrompt(){
  const el=document.getElementById('rotate-prompt');
  const skip=document.getElementById('rot-skip');
  if(!el||!skip)return;
  skip.addEventListener('pointerdown',e=>{e.preventDefault();el.classList.add('dismissed');});
  const rearm=()=>el.classList.remove('dismissed');
  window.addEventListener('orientationchange',rearm);
  if(screen.orientation)screen.orientation.addEventListener?.('change',rearm);
}
...
function initMobileAutoFullscreen(){
  ...
  document.addEventListener('fullscreenchange',()=>{
    if(!document.fullscreenElement&&!document.webkitFullscreenElement)arm();
  });
}
function startSim(){
